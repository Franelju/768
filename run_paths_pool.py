...
worker_num = 12  # how many workers, None means default (max workers)
...
if __name__ == "__main__":
    ...
    result = pool.map(
        run_many_paths, inputs
    )  # maps function calls to the workers, by iterating over inputs
    ...
    df = pd.DataFrame(
        {
            ...
            "t": tpts,
            "X": result[k][0],
            "X_int": np.r_[0, 0, result[k][2]],
            "X_inc": result[k][4],
            "dGx": result[k][8],
            "Y": result[k][1],
            "Y_int": np.r_[0, 0, result[k][3]],
            "Y_inc": result[k][5],
            "dGy": result[k][9],
            "seed": seeds[k],
        }
